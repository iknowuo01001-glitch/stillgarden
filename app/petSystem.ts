...
export type Visitor = { speciesId:PetSpeciesId; arrivedAt:number; leavesAt:number; visits:number; trust:number; greeted:boolean };
export type PetSave = {
...
export function emptyPetSave(now=Date.now()): PetSave {
  return { version:1,pets:[],activePetId:null,eggs:[],discovered:[],visitor:null,nextVisitorAt:now+2*60_000,lurePlant:null,rarePity:0,nestMisses:0,selectedPetId:null };
}
export function loadPetSave(): PetSave {
  if (typeof window==="undefined") return emptyPetSave();
  try { const raw=localStorage.getItem(PET_SAVE_KEY); if(!raw) return emptyPetSave(); const saved=JSON.parse(raw) as Partial<PetSave>; return { ...emptyPetSave(), ...saved, pets:Array.isArray(saved.pets)?saved.pets:[], eggs:Array.isArray(saved.eggs)?saved.eggs:[], discovered:Array.isArray(saved.discovered)?saved.discovered:[] }; } catch { return emptyPetSave(); }
}
...
function rollLustre(seed=Math.random()){ const r=typeof seed==="number"?seed:Math.random(); if(r<1/10000)return "eclipse"; if(r<1/1800)return "prismatic"; if(r<1/180)return "shimmer"; if(r<1/20)return "dappled"; return "plain"; }
function makeGenes(seed:number, a?:PetGenes, b?:PetGenes): PetGenes {
  ...
  let lustre:Lustre=rollLustre(hash(seed+909)); if(a&&b&&hash(seed+811)<.18) lustre=hash(seed+812)<.5?a.lustre:b.lustre;
...
export function createPet(speciesId:PetSpeciesId, opts?:{parents?:Gardenkin[];generation?:number;seed?:number;name?:string}):Gardenkin{
  const seed=opts?.seed??Math.random()*1e9; const parents=opts?.parents??[]; const sp=speciesOf(speciesId);
  return { id:uid(),speciesId,name:opts?.name??sp.name,bornAt:Date.now(),generation:opts?.generation??1,parents:parents.map(p=>p.id),xp:0,bond:0,genes:makeGenes(seed,parents[0]?.genes,parents[1]?.genes),aptitude:1+Math.floor(hash(seed+212)*5),care:{tend:0,cuddle:0,forage:0,grow:0},activeKnacks:[] };
}
...
export function addPetProgress(save:PetSave, petId:string, xp:number, bond:number, care: keyof Care):PetSave { return {...save,pets:save.pets.map(p=>p.id!==petId?p:{...p,xp:p.xp+xp,bond:p.bond+bond,care:{...p.care,[care]:p.care[care]+xp}})}; }
...
function weightedSpecies(stage:number,lure:string|null,pity:number){ const pool=unlockedWild(stage); const weighted:{id:PetSpeciesId,w:number}[]=pool.map(s=>{ const base=[0,55,28,13,5,1.5,.3][s.rarity]||1; const lureBoost=lure&&s.affinity.includes(lure as never)?3:1; const pityBoost=s.rarity>=4?1+pity*.14:1; return {id:s.id,w:base*lureBoost*pityBoost};}); const total=weighted.reduce((a,b)=>a+b.w,0); let r=Math.random()*total; for(const item of weighted){r-=item.w;if(r<=0)return item.id;} return weighted[0]?.id??"mossbun"; }
export function processVisitor(save:PetSave,stage:number,now=Date.now()):PetSave { if(save.visitor&&save.visitor.leavesAt>now)return save; if(save.visitor&&save.visitor.leavesAt<=now) save={...save,visitor:null}; if(now<save.nextVisitorAt)return save; const id=weightedSpecies(stage,save.lurePlant,save.rarePity); const sp=speciesOf(id); const previousVisits=0; const rarePity=sp.rarity>=4?0:save.rarePity+1; return {...save,visitor:{speciesId:id,arrivedAt:now,leavesAt:now+45*60_000,visits:previousVisits+1,trust:0,greeted:false},rarePity,nextVisitorAt:now+(18+Math.floor(Math.random()*42))*60_000}; }
export function greetVisitor(save:PetSave):PetSave { if(!save.visitor||save.visitor.greeted)return save; return {...save,visitor:{...save.visitor,greeted:true,trust:save.visitor.trust+1}}; }
export function inviteVisitor(save:PetSave):PetSave { const v=save.visitor;if(!v)return save; const sp=speciesOf(v.speciesId); const need=[0,1,2,3,4,5,6][sp.rarity]||3; if(v.trust<need)return save; const pet=createPet(v.speciesId); return {...save,pets:[...save.pets,pet],discovered:save.discovered.includes(v.speciesId)?save.discovered:[...save.discovered,v.speciesId],visitor:null,selectedPetId:pet.id,activePetId:save.activePetId??pet.id,nextVisitorAt:Date.now()+25*60_000}; }
...
export function maybeFindNest(save:PetSave,stage:number):{save:PetSave;found:PetEgg|null}{ const active=save.pets.find(p=>p.id===save.activePetId); const seednose=active?.activeKnacks.includes("seednose")??false; const chance=.09+(seednose?.035:0)+Math.min(.11,save.nestMisses*.012); if(Math.random()>chance) return {save:{...save,nestMisses:save.nestMisses+1},found:null}; const speciesId=weightedSpecies(stage,save.lurePlant,save.rarePity); const sp=speciesOf(speciesId); const baseMinutes=[0,8,20,45,90,180,300][sp.rarity]||30; const egg:PetEgg={id:uid("egg"),speciesId,readyAt:Date.now()+baseMinutes*60_000,createdAt:Date.now(),source:"found beneath a settled garden",odds:Math.max(4,Math.round(1/(chance*(1/Math.max(1,sp.rarity))))),geneSeed:Math.random()*1e9,parents:[],generation:1}; return {save:{...save,eggs:[...save.eggs,egg],nestMisses:0},found:egg}; }
...
export function breedPets(save:PetSave,aId:string,bId:string):{save:PetSave;ok:boolean;message:string}{ const a=save.pets.find(p=>p.id===aId),b=save.pets.find(p=>p.id===bId); if(!a||!b||!canBreed(a,b))return {save,ok:false,message:"Both companions must be level 25+ with bond 3+."}; const cost=breedingCost(a,b); const main=readMainSave(); if(!main||Number(main.dew||0)<cost)return {save,ok:false,message:`Need ${cost} Dew for nesting materials.`}; if(save.eggs.length>=4)return {save,ok:false,message:"All four nest spaces are occupied."}; main.dew-=cost; writeMainSave(main); const hybrid=hybridResult(a.speciesId,b.speciesId); let speciesId:PetSpeciesId; const r=Math.random(); if(hybrid&&r<.16)speciesId=hybrid; else speciesId=r<.58?a.speciesId:b.speciesId; const sp=speciesOf(speciesId); const generation=Math.max(a.generation,b.generation)+1; const egg:PetEgg={id:uid("egg"),speciesId,readyAt:Date.now()+(45+sp.rarity*35)*60_000,createdAt:Date.now(),source:`bred from ${a.name} × ${b.name}`,odds:hybrid&&speciesId===hybrid?6:2,geneSeed:Math.random()*1e9,parents:[a.id,b.id],generation}; return {save:{...save,eggs:[...save.eggs,egg]},ok:true,message:hybrid&&speciesId===hybrid?"The egg carries an unfamiliar lineage.":"A new egg is resting in the nest."}; }
