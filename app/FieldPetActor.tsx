...
export default function FieldPetActor({pet,onNotify}:Props){
  ...
  function setPose(pose:Pose,extra?:Partial<ActorState>){
    setActor(current=>({...current,pose,...extra}));
  }

  function hideActor(){
    setActor(current=>({...current,visible:false,pose:"idle"}));
  }
  ...
  async function moveToCell(cell:HTMLButtonElement,pose:Pose="walk",token=sequence.current){
    ...
    const next=pointFor(cell);
    const dist=distance(actorPos.current,next);
  ...
  function chooseTargets(origin:number,current:Gardenkin){
    ...
    let originButton=document.querySelector<HTMLButtonElement>(`button[data-cell='${origin}']`);
    if(!originButton)originButton=buttons[0]??null;
    ...
    const metric=(button:HTMLButtonElement)=>{
      const r=button.getBoundingClientRect();
      return Math.hypot(r.left+r.width/2-centre.x,r.top+r.height/2-centre.y);
    };
    ...
    if(currentInfo.assist==="line"){
      const sameBand=buttons.filter(button=>{
        const r=button.getBoundingClientRect();
        return Math.abs(r.top+r.height/2-centre.y)<Math.max(18,r.height*.9);
      }).sort((a,b)=>metric(a)-metric(b));
      if(sameBand.length)return sameBand.slice(0,2+extra);
    }
    if(currentInfo.assist==="burst"){
      return buttons.sort((a,b)=>metric(a)-metric(b)).slice(0,2+extra);
    }
  ...
  async function pokeCell(button:HTMLButtonElement,index:number,token:number){
    if(!(await moveToCell(button,"walk",token)))return;
  ...
  async function runAssist(origin:number,current:Gardenkin){
    ...
    const targets=chooseTargets(origin,current);
    ...
    for(let i=0;i<targets.length;i++){
      if(token!==sequence.current)break;
      await pokeCell(targets[i],i,token);
    }
    ...
    assisting.current=false;
  }

  useEffect(()=>{
    const onPointerDown=(event:PointerEvent)=>{
      const target=event.target;
      if(target instanceof Element&&target.closest("[data-cell]")){
  ...
  useEffect(()=>{
    ...
    const pulse=window.setInterval(()=>{
      const cells=validCells();
      if(!cells.length)hideActor(); else if(!actor.visible)placeActor(true);
    },900);
  ...
  useEffect(()=>{
    ...
    const roam=async()=>{
      ...
      if(ok){
        ...
        setPose(roll<.22?"rest":roll<.48?"watch":"idle");
      }
      moving.current=false;
    };
  ...
  useEffect(()=>{
    ...
    const observer=new MutationObserver(mutations=>{
      ...
      for(const mutation of mutations){
        ...
        if(el.dataset.restored!=="true")continue;
      ...
      const save=loadPetSave();
      const current=activePet(save);
      if(!current||!gained)return;
      const next=addPetProgress(save,current.id,assisting.current?0:gained,assisting.current?0:gained*.09,"tend");
      savePetSave(next);
      if(!assisting.current){
        charge.current+=gained;
        const refreshed=activePet(next);
        if(refreshed&&charge.current>=petAssistEvery(refreshed)){
          charge.current=0;
          void runAssist(newest??0,refreshed);
        }
      }
      ...
      if(done){
        ...
        if(completedKey.current!==plotText){
          ...
          let currentSave=loadPetSave();
          const companion=activePet(currentSave);
          if(companion)currentSave=addPetProgress(currentSave,companion.id,36,5,"forage");
          const nest=maybeFindNest(currentSave,stage);
          savePetSave(nest.save);
          if(nest.found)onNotify(`${companion?.name??"Your companion"} found a warm nest.`);
          if(companion){
            setPose("celebrate",{pulse:Date.now()});
            window.setTimeout(()=>setPose("idle"),reduceMotion?90:650);
          }
        }
      }
    });
  ...
  return <motion.div
    className={styles.actorMover}
    aria-hidden="true"
    initial={false}
    animate={{x:actor.x,y:actor.y,opacity:actor.visible?1:0}}
    transition={{duration:actor.duration,ease:[.22,.78,.28,1]}}
  >
    <div
      className={styles.fieldActor}
      data-body={info.body}
      data-pose={actor.pose}
      data-facing={actor.facing}
      data-lustre={pet.genes.lustre}
      style={{"--actor-coat":coat,"--actor-accent":accent,"--actor-pattern":pet.genes.pattern} as React.CSSProperties}
    >
      <div className={styles.actorVisual}>
        <i className={styles.actorShadow}/>
        <i className={styles.actorTail}/>
        <i className={styles.actorBody}/>
        <i className={styles.actorBelly}/>
        <i className={styles.actorPawLeft}/>
        <i className={styles.actorPawRight}/>
        <i className={styles.actorHead}/>
        <i className={`${styles.actorEar} ${styles.actorEarLeft}`}/>
        <i className={`${styles.actorEar} ${styles.actorEarRight}`}/>
        <i className={styles.actorFace}><b/><b/><em/></i>
        <i className={styles.actorMark}/>
        <i className={styles.actorBloom}/>
        <i className={styles.actorWing}/>
        <i className={styles.actorSpark}/>
      </div>
